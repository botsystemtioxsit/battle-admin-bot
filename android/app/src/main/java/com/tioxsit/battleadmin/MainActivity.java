...
public class MainActivity extends Activity {
    ...
    //
    // Было опробовано скачивание+самоустановка через DownloadManager +
    // системный установщик (требует REQUEST_INSTALL_PACKAGES) — именно эта
    // связка ("сторонний APK не из Google Play, который сам умеет ставить
    // другие APK") заблокировала сам APK как "вредоносное приложение" в
    // Google Play Защите. Обходной путь через Google Play Console стоит
    // денег (принцип проекта — ничего платного), поэтому решили: кнопка
    // просто открывает прямую ссылку на APK на GitHub в браузере, человек
    // сам тапает по скачанному файлу — так же, как при самой первой
    // установке. REQUEST_INSTALL_PACKAGES приложению не нужен вообще.
