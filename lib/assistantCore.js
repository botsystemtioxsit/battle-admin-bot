// Общая логика ИИ-ассистента админ-панели — см. вкладку "Ассистент" в
// src/admin.html. Используется ДВУМЯ разными способами запуска (оба берут
// этот же файл, чтобы не держать две копии логики):
//  - .github/scripts/run-assistant.js — через GitHub Actions
//    (workflow_dispatch), секрет ANTHROPIC_API_KEY хранится в Settings →
//    Secrets репозитория. Ответ приходит с задержкой (обычно 20–60 секунд
//    на запуск раннера) — активный способ по умолчанию, не требует
//    заводить аккаунт ни на каком стороннем сайте.
//  - api/claude-assistant.js — serverless-функция на Vercel, секрет в
//    переменных окружения проекта. Отвечает мгновенно, но требует один раз
//    завести проект на vercel.com. Держим как альтернативу на будущее.
// В обоих случаях ключ никогда не попадает в клиентский код — только в
// секреты соответствующей платформы.
//
// Права ассистента: доступ к тем же самым RTDB-действиям, что и у
// superadmin в этой панели (см. TOOLS ниже). Инструменты делятся на два
// рода:
...
//    конфиг-флаги) — ИЗМЕНЯЮЩИЕ. Ассистент их только ПРЕДЛАГАЕТ: вызывающий
//    код приостанавливает цикл и возвращает предложенные действия клиенту,
...
//    (см. resolveToolUseBlocks ниже и renderAssistantPendingActions в
...
const { dbGet, dbSet, dbUpdate, dbDelete, dbPush } = require('./firebaseRest');
...
Учти также, что твой ответ может прийти с задержкой в десятки секунд (это
нормально, не повод для беспокойства и не повод писать об этом человеку).
...
async function logAction(actionType, targetKey, targetName, targetPlayerId, details) {
  try {
    ...
    console.error('assistant: не удалось записать в журнал действий', err);
  }
}
...
// Прогоняет полный цикл tool-use, начиная с уже собранных `messages`
// (опционально с `decisions` для хода ассистента, которым messages
// заканчивается — см. resolveToolUseBlocks). Возвращает финальный ответ
// либо awaitingConfirmation с pendingActions, если упёрлись в
// неподтверждённые изменяющие действия. Общая точка входа для обоих
// способов запуска (Vercel/Actions) — сама логика цикла не должна
// расходиться между ними.
async function runAssistantLoop(initialMessages, initialDecisions, apiKey) {
  ...
  if (initialDecisions) {
    ...
    if (resolved.awaiting) {
      return { reply: '', actions, messages, awaitingConfirmation: true, pendingActions: resolved.pendingActions };
    }
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    if (!toolUseBlocks.length) {
      ...
      return { reply: text, actions, messages: [...messages, { role: 'assistant', content: resp.content }] };
    }
    ...
    if (resolved.awaiting) {
      const text = resp.content.filter((b) => b.type === 'text').map((b) => b.text).join('\n');
      return { reply: text, actions, messages, awaitingConfirmation: true, pendingActions: resolved.pendingActions };
    }
    messages.push({ role: 'user', content: resolved.toolResults });
  }
...
module.exports = { TOOLS, SYSTEM_PROMPT, HANDLERS, READ_ONLY_TOOL_NAMES, MAX_TURNS, resolveToolUseBlocks, callClaude, runAssistantLoop };
